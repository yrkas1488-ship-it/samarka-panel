...
export default function NodeFormModal({
  ...
  const [workerMode, setWorkerMode] = useState<'cdn' | 'double_vpn'>('cdn');

  const onWorkerRoleSelect = (modeValue: 'cdn' | 'double_vpn') => {
    setWorkerMode(modeValue);
    if (modeValue === 'double_vpn') {
      form.setFieldsValue({
        remark: 'Второстепенный сервер: Двойной VPN (Exit/Relay)',
        inboundSyncMode: 'selected',
      });
    } else {
      form.setFieldsValue({
        remark: 'Второстепенный сервер: Через CDN (Origin)',
        inboundSyncMode: 'all',
      });
    }
  };

  const parseAndApplyJoinLink = (link: string) => {
    ...
    try {
      ...
      const basePath = params.get('basePath') || parsedUrl.pathname || '/';
      const scheme = (params.get('scheme') || (parsedUrl.protocol ? parsedUrl.protocol.replace(':', '') : 'https')) as 'http' | 'https';

      form.setFieldsValue({
        name: form.getFieldValue('name') || `Node-${addr || 'Remote'}`,
      ...
      if (scheme === 'http') form.setFieldValue('tlsVerifyMode', 'skip');

      messageApi.success('Параметры ноды успешно извлечены из ссылки!');
    } catch {
      messageApi.error('Неверный формат ссылки подключения');
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          {mode === 'add' && (
            <div style={{ background: 'rgba(22, 119, 255, 0.06)', border: '1px dashed #1677ff', borderRadius: 8, padding: 12, marginBottom: 16 }}>
              <div style={{ fontWeight: 600, marginBottom: 6, color: '#1677ff' }}>
                ⚡ Вставить ссылку от Главного сервера (Master)
              </div>
              <Input.Search
                placeholder="Вставьте ссылку вида samarka://join?... или https://ip:port/path?token=..."
                enterButton="Заполнить"
                onSearch={parseAndApplyJoinLink}
              />
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 12, color: 'rgba(128,128,128,0.85)' }}>Режим сервера:</span>
                <Radio.Group
                  size="small"
                  value={workerMode}
                  onChange={(e) => onWorkerRoleSelect(e.target.value)}
                >
                  <Radio.Button value="cdn">🛡️ Через CDN (Origin)</Radio.Button>
                  <Radio.Button value="double_vpn">🔀 Двойной VPN (Exit/Relay)</Radio.Button>
                </Radio.Group>
              </div>
            </div>
          )}
