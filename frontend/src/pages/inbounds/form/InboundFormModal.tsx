...
import YandexCloudModal from './YandexCloudModal';
...
export default function InboundFormModal({
  ...
  const applyPreset = (preset: 'vless_reality_grpc' | 'vless_reality_tcp' | 'vless_xhttp_yandex' | 'vless_ws_cdn') => {
    if (preset === 'vless_reality_grpc') {
      form.setFieldsValue({
        protocol: 'vless',
        port: 8443,
        remark: 'Samarka-Reality-gRPC',
        streamSettings: {
          network: 'grpc',
          security: 'reality',
          grpcSettings: {
            serviceName: `grpc-${RandomUtil.randomLowerAndNum(6)}`,
            multiMode: true,
          },
          realitySettings: {
            show: false,
            xver: 0,
            target: 'xn--80aak4aef1h.xn--p1ai:443',
            serverNames: ['xn--80aak4aef1h.xn--p1ai', 'максвайб.рф'],
            settings: {
              fingerprint: 'randomized',
              spiderX: '/',
            },
            shortIds: [RandomUtil.randomSeq(8, { type: 'hex' })],
          },
        },
      });
      genRealityKeypair();
      messageApi.success('Применен шаблон: VLESS + Reality + gRPC (порт 8443, uTLS randomized, максвайб.рф)');
    } else if (preset === 'vless_reality_tcp') {
      form.setFieldsValue({
        protocol: 'vless',
        port: 8443,
        remark: 'Samarka-Reality-TCP',
        streamSettings: {
          network: 'tcp',
          security: 'reality',
      ...
      genRealityKeypair();
      messageApi.success('Применен шаблон: VLESS + Reality + TCP (порт 8443, uTLS randomized, максвайб.рф)');
    } else if (preset === 'vless_xhttp_yandex') {
      form.setFieldsValue({
        protocol: 'vless',
        port: 443,
      ...
      messageApi.success('Применен шаблон: XHTTP Yandex Cloud CDN (порт 443, OPTIONS, X-Cache padding)');
    } else if (preset === 'vless_ws_cdn') {
      form.setFieldsValue({
        protocol: 'vless',
        port: 443,
        remark: 'Samarka-WS-CDN',
        streamSettings: {
          network: 'ws',
          security: 'none',
          wsSettings: {
      ...
      messageApi.success('Применен шаблон: VLESS + WebSocket CDN (порт 443)');
    }
  };
  ...
  const basicTab = (
    <>
      {mode === 'add' && (
        <Card
          size="small"
          style={{ marginBottom: 16, background: 'rgba(255, 140, 0, 0.05)', borderColor: 'rgba(255, 140, 0, 0.25)' }}
          title={
            <Space>
              <ThunderboltOutlined style={{ color: '#fa8c16' }} />
              <span style={{ fontWeight: 600 }}>Быстрые шаблоны Samarka</span>
            </Space>
          }
          extra={
            <Button size="small" type="link" icon={<QuestionCircleOutlined />} onClick={() => setYandexModalOpen(true)}>
              Инструкция RU CDN
            </Button>
          }
        >
          <Space wrap>
            <Button size="small" onClick={() => applyPreset('vless_reality_grpc')}>
              🚀 VLESS + Reality + gRPC (8443)
            </Button>
            <Button size="small" onClick={() => applyPreset('vless_reality_tcp')}>
              ⚡ VLESS + Reality + TCP (8443)
            </Button>
            <Button size="small" type="primary" ghost onClick={() => applyPreset('vless_xhttp_yandex')}>
              🇷🇺 VLESS + XHTTP (Yandex CDN 443)
            </Button>
            <Button size="small" onClick={() => applyPreset('vless_ws_cdn')}>
              ☁️ VLESS + WS CDN (443)
            </Button>
          </Space>
        </Card>
  ...
  return (
    <>
      ...
      <YandexCloudModal open={yandexModalOpen} onClose={() => setYandexModalOpen(false)} />
    </>
  );
}
