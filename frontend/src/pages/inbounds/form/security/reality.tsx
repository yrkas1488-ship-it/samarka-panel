...
import { Alert, Button, Collapse, Descriptions, Divider, Form, Input, InputNumber, Radio, Select, Space, Switch } from 'antd';
...
export default function RealityForm({
  ...
  const currentTarget = Form.useWatch(['streamSettings', 'realitySettings', 'target'], form);

  let activeMask = 'custom';
  if (currentTarget === 'xn--80aak4aef1h.xn--p1ai:443') {
    activeMask = 'maxvibe_rf';
  } else if (currentTarget === 'nemaxvibe.lol:443') {
    activeMask = 'nemaxvibe_lol';
  }

  const handleMaskChange = (value: string) => {
    if (value === 'maxvibe_rf') {
      form.setFieldValue(['streamSettings', 'realitySettings', 'target'], 'xn--80aak4aef1h.xn--p1ai:443');
      form.setFieldValue(['streamSettings', 'realitySettings', 'serverNames'], ['xn--80aak4aef1h.xn--p1ai', 'максвайб.рф']);
    } else if (value === 'nemaxvibe_lol') {
      form.setFieldValue(['streamSettings', 'realitySettings', 'target'], 'nemaxvibe.lol:443');
      form.setFieldValue(['streamSettings', 'realitySettings', 'serverNames'], ['nemaxvibe.lol']);
    } else {
      if (activeMask !== 'custom') {
        form.setFieldValue(['streamSettings', 'realitySettings', 'target'], '');
        form.setFieldValue(['streamSettings', 'realitySettings', 'serverNames'], []);
      }
    }
  };
  ...
  return (
    <>
      ...
      <Form.Item label="Маскировка (Camouflage)">
        <Radio.Group
          value={activeMask}
          onChange={(e) => handleMaskChange(e.target.value)}
          buttonStyle="solid"
        >
          <Radio.Button value="maxvibe_rf">🌐 максвайб.рф</Radio.Button>
          <Radio.Button value="nemaxvibe_lol">🌐 nemaxvibe.lol</Radio.Button>
          <Radio.Button value="custom">✏️ Предложить свой...</Radio.Button>
        </Radio.Group>
      </Form.Item>
